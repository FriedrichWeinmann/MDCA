...
#>
	...
	process
	{
		try { $uri = '{0}{1}' -f (Get-PSFConfigValue -FullName 'MDCA.Client.Uri' -NotNull), $FunctionName }
		catch { $PSCmdlet.ThrowTerminatingError($_) }
		$header = @{ }
		
		#region Authentication
		$unprotectedToken = Get-PSFConfigValue -FullName 'MDCA.Client.UnprotectedToken'
		$protectedToken = Get-PSFConfigValue -FullName 'MDCA.Client.ProtectedToken'
