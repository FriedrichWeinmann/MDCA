...
$parentModule = 'MDCA'
if (-not $ModuleName) { $ModuleName = 'MDCA.Client' }
...
$publishRoot = Join-Path -Path $WorkingDirectory -ChildPath 'publish\MDCA'
...
$originalManifestData = Import-PowerShellDataFile -Path "$publishRoot\MDCA.psd1"
$prereqHash = @{
